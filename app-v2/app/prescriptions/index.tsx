...
export default function PrescriptionsScreen() {
    ...
    const handleViewPress = (item: Prescription) => {
        import('sonner-native').then(({ toast }) => {
            toast.info(`Viewing ${item.type} functionality coming soon.`);
        });
    };

    const handleDownloadPress = (item: Prescription) => {
        import('sonner-native').then(({ toast }) => {
            toast.success(`${item.type} download started.`);
        });
    };
    ...
    return (
        <SafeAreaView className="flex-1 bg-white">
            ...
            <View className="flex-row items-center px-4 py-3 border-b border-gray-100">
                ...
                <Text className="text-xl font-bold text-gray-900">Medical History</Text>
            </View>
            ...
            <View className="flex-1 px-4 pt-4">
                {isLoading ? (
                    ...
                    <View className="flex-1 justify-center items-center">
                        ...
                        <Text className="text-gray-900 text-lg font-bold mb-2">No Records Found</Text>
                        <Text className="text-gray-500 text-center px-8">
                            You don't have any medical history or prescriptions available yet.
                        </Text>
                    </View>
                ) : (
                    <FlatList
