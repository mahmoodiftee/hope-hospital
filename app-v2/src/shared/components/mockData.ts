export const images: Record<string, any> = {
    arrowBack: require("../../../assets/icons/arrow-back.png"),
    arrowDown: require("../../../assets/icons/arrow-down.png"),
    arrowRight: require("../../../assets/icons/arrow-right.png"),
    heart: require("../../../assets/images/heart.png"),
    kidney: require("../../../assets/images/kidney.png"),
    liver: require("../../../assets/images/liver.png"),
    lungs: require("../../../assets/images/lungs.png"),
    dental: require("../../../assets/images/teeth.png"),
    ambulance: require("../../../assets/icons/ambulance.png"),
    opd: require("../../../assets/icons/opd.png"),
    texture: require("../../../assets/images/texture.png"),
    kidneyIcon: require("../../../assets/assets/images/kidneyIcon.png"),
    liverIcon: require("../../../assets/assets/images/liverIcon.png"),
};

export const topDoctorList = [
    {
        "name": "Dr. Shabnam Akhter",
        "name_bn": "ডাঃ শবনম আক্তার",
        "specialty": "Kidney",
        "specialty_bn": "কিডনি",
        "hourlyRate": 2100,
        "image": "https://i.ibb.co/mrybFMsP/doctor-white-coat-using-digital-tablet-reading-medical-data-gadget-working-hospital-standin.jpg",
        "experience": "Dr. Shabnam Akhter is a renowned nephrologist with 13 years of experience in kidney care and transplantation. She has expertise in managing chronic kidney disease, dialysis, and kidney transplant procedures. Her compassionate approach and clinical excellence have helped hundreds of patients maintain healthy kidney function.",
        "experience_bn": "ডাঃ শবনম আক্তার কিডনি যত্ন এবং প্রতিস্থাপনে ১৩ বছরের অভিজ্ঞতা সম্পন্ন একজন বিখ্যাত নেফ্রোলজিস্ট। দীর্ঘস্থায়ী কিডনি রোগ, ডায়ালাইসিস এবং কিডনি প্রতিস্থাপন পদ্ধতি পরিচালনায় তার দক্ষতা রয়েছে। তার সহানুভূতিশীল দৃষ্টিভঙ্গি এবং ক্লিনিকাল শ্রেষ্ঠত্ব শত শত রোগীকে স্বাস্থ্যকর কিডনির কার্যকারিতা বজায় রাখতে সাহায্য করেছে।",
        "specialties": [
            "Kidney Transplantation - Comprehensive transplant care",
            "Dialysis Management - Both hemodialysis and peritoneal dialysis",
            "Chronic Kidney Disease - Early intervention and management"
        ],
        "specialties_bn": [
            "কিডনি প্রতিস্থাপন - ব্যাপক প্রতিস্থাপন যত্ন",
            "ডায়ালাইসিস ব্যবস্থাপনা - হেমোডায়ালাইসিস এবং পেরিটোনিয়াল ডায়ালাইসিস উভয়ই",
            "দীর্ঘস্থায়ী কিডনি রোগ - প্রাথমিক চিকিৎসা এবং ব্যবস্থাপনা"
        ],
        "$id": "688a76c2001cf58a30d1",
    },
    {
        "name": "Dr. Habibul Bashar",
        "name_bn": "ডাঃ হাবিবুল বাশার",
        "specialty": "General Medicine",
        "specialty_bn": "সাধারণ ঔষধ",
        "hourlyRate": 1550,
        "image": "https://i.ibb.co/GjqqrMB/portrait-smiling-young-doctors-standing-together-portrait-medical-staff-inside-modern-hospital-smili.jpg",
        "experience": "Dr. Habibul Bashar is a dedicated general physician with 13 years of experience in emergency medicine and acute care. He provides comprehensive emergency medical services and urgent care treatment. His quick decision-making skills and broad medical knowledge have been crucial in managing medical emergencies and providing immediate patient care.",
        "experience_bn": "ডাঃ হাবিবুল বাশার জরুরি চিকিৎসা এবং তীব্র যত্নে ১৩ বছরের অভিজ্ঞতা সম্পন্ন একজন নিবেদিত সাধারণ চিকিৎসক। তিনি ব্যাপক জরুরি চিকিৎসা সেবা এবং জরুরি যত্নের চিকিৎসা প্রদান করেন। চিকিৎসা জরুরি অবস্থা সামলানো এবং তাৎক্ষণিক রোগীর যত্ন প্রদানে তার দ্রুত সিদ্ধান্ত নেওয়ার দক্ষতা এবং ব্যাপক চিকিৎসা জ্ঞান অত্যন্ত গুরুত্বপূর্ণ।",
        "specialties": [
            "Emergency Medicine - Acute medical care and trauma treatment",
            "Urgent Care - Immediate medical attention for non-life-threatening conditions",
            "Critical Care Support - Emergency stabilization and patient management"
        ],
        "specialties_bn": [
            "জরুরি চিকিৎসা - তীব্র চিকিৎসা সেবা এবং ট্রমা চিকিৎসা",
            "জরুরি যত্ন - জীবনের জন্য হুমকিস্বরূপ নয় এমন অবস্থার জন্য তাৎক্ষণিক চিকিৎসা মনোযোগ",
            "ক্রিটিকাল কেয়ার সাপোর্ট - জরুরি স্থিতিশীলতা এবং রোগীর ব্যবস্থাপনা"
        ],
        "$id": "688a76cb003cb3b87063",
    },
    {
        "name": "Dr. Mahmuda Khatun",
        "name_bn": "ডাঃ মাহমুদা খাতুন",
        "specialty": "Kidney",
        "specialty_bn": "কিডনি",
        "hourlyRate": 1850,
        "image": "https://i.ibb.co/JjTdc4N8/woman-doctor-wearing-lab-coat-with-stethoscope-isolated.jpg",
        "experience": "Dr. Mahmuda Khatun is a compassionate nephrologist with 9 years of experience in women's kidney health and pregnancy-related kidney issues. She specializes in treating kidney diseases during pregnancy, preeclampsia, and reproductive health-related kidney problems. Her focus on women's nephrology has filled an important healthcare gap.",
        "experience_bn": "ডাঃ মাহমুদা খাতুন একজন সহানুভূতিশীল নেফ্রোলজিস্ট যিনি নারীদের কিডনি স্বাস্থ্য এবং গর্ভাবস্থা সম্পর্কিত কিডনি সমস্যায় ৯ বছরের অভিজ্ঞতা সম্পন্ন। তিনি গর্ভাবস্থায় কিডনি রোগ, প্রিক্ল্যাম্পসিয়া এবং প্রজনন স্বাস্থ্য সম্পর্কিত কিডনি সমস্যার চিকিৎসায় বিশেষজ্ঞ। নারীদের নেফ্রোলজিতে তার ফোকাস একটি গুরুত্বপূর্ণ স্বাস্থ্যসেবা শূন্যতা পূরণ করেছে।",
        "specialties": [
            "Pregnancy-Related Kidney Disease - Kidney problems during pregnancy",
            "Women's Nephrology - Female-specific kidney health issues",
            "Preeclampsia Management - Pregnancy-induced hypertension and kidney involvement"
        ],
        "specialties_bn": [
            "গর্ভাবস্থা সম্পর্কিত কিডনি রোগ - গর্ভাবস্থায় কিডনির সমস্যা",
            "নারীদের নেফ্রোলজি - নারীদের জন্য নির্দিষ্ট কিডনি স্বাস্থ্য সমস্যা",
            "প্রিক্ল্যাম্পসিয়া ব্যবস্থাপনা - গর্ভাবস্থায় প্ররোচিত উচ্চ রক্তচাপ এবং কিডনির সমস্যা"
        ],
        "$id": "688a76cb0028e0327289",
    },
    {
        "name": "Dr. Nayeem Chowdhury",
        "name_bn": "ডাঃ নাঈম চৌধুরী",
        "specialty": "Lungs",
        "specialty_bn": "ফুসফুস",
        "hourlyRate": 1800,
        "image": "https://images.pexels.com/photos/32115962/pexels-photo-32115962.jpeg",
        "experience": "Dr. Nayeem Chowdhury is a skilled pulmonologist with 12 years of experience in occupational lung diseases and environmental respiratory health. He specializes in treating work-related lung conditions, asbestos exposure effects, and environmental lung damage. His expertise in occupational health has been vital for industrial workers.",
        "experience_bn": "ডাঃ নাঈম চৌধুরী পেশাগত ফুসফুসের রোগ এবং পরিবেশগত শ্বাসযন্ত্রের স্বাস্থ্যে ১২ বছরের অভিজ্ঞতা সম্পন্ন একজন দক্ষ পালমোনোলজিস্ট। তিনি কর্ম সম্পর্কিত ফুসফুসের অবস্থা, অ্যাসবেস্টস এক্সপোজারের প্রভাব এবং পরিবেশগত ফুসফুসের ক্ষতির চিকিৎসায় বিশেষজ্ঞ। শিল্প শ্রমিকদের জন্য পেশাগত স্বাস্থ্যে তার দক্ষতা গুরুত্বপূর্ণ।",
        "specialties": [
            "Occupational Lung Disease - Work-related respiratory conditions",
            "Environmental Respiratory Health - Pollution-related lung damage",
            "Pulmonary Rehabilitation - Breathing exercises and lung function improvement"
        ],
        "specialties_bn": [
            "পেশাগত ফুসফুসের রোগ - কর্ম সম্পর্কিত শ্বাসকষ্টজনিত অবস্থা",
            "পরিবেশগত শ্বাসযন্ত্রের স্বাস্থ্য - দূষণ সম্পর্কিত ফুসফুসের ক্ষতি",
            "পালমোনারি রিহ্যাবিলিটেশন - শ্বাস-প্রশ্বাসের ব্যায়াম এবং ফুসফুসের কার্যকারিতা উন্নতি"
        ],
        "$id": "688a76cb0011e958d1fc",
    }
];
